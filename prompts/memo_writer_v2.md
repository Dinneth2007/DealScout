...
Changelog: v2 — DeepSeek's OpenAI-compatible endpoint rejects the SDK's
json_schema response_format ("This response_format type is unavailable now"),
so structured output is produced via explicit JSON instruction + boundary
parse (same pattern as intake v3). v1 left intact.
